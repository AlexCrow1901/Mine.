...
window.MineCompanion = (function () {
  ...
  var CONTENT_APPS = [
    ...
    // —— 温暖陪伴 ——
    { id: "warm-cafe",       icon: "coffee",   title: "雾都咖啡馆", desc: "一杯虚拟热饮", soon: true },
  ...
  function getDailyQuote() {
    /* 每天固定种子随机抽取一句，同一天打开看到的是同一句 */
    var now = new Date();
    var dayIndex = Math.floor(now.getTime() / 86400000);
    // 用日期作为种子做伪随机，确保同一天结果一致
    var seed = dayIndex;
    var rand = function () {
  ...
  function cardHTML(app) {
    ...
    return '<div class="comp-card' + wide + accent + '" role="button" tabindex="0" data-app="' + app.id + '">' +
      ...
      '<div class="comp-card-icon">' + I.svg(app.icon, 22) + '</div>' +
  ...
  function viewCompanion() {
    ...
    CONTENT_APPS.filter(function (a) {
      return ["warm-cafe", "quiet-flame", "companion-heart"].indexOf(a.id) >= 0;
    }).forEach(function (a) { html += cardHTML(a); });
