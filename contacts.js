...
window.MineContacts = (function () {
    ...
    try {
      ...
      if (raw) {
        ...
        state.contacts.forEach(function (c) {
          ...
          // 主动消息时间区间（分钟），默认 1~720 分钟（1分钟~12小时）
          if (c.activeMinMin === undefined) c.activeMinMin = 1;
          if (c.activeMaxMin === undefined) c.activeMaxMin = 720;
          // 去重：清理历史遗留的重复字卡
  ...
  function viewProfile(data) {
    ...
    // 主动消息设置区
    var actMinMin = c.activeMinMin !== undefined ? c.activeMinMin : 1;
    var actMaxMin = c.activeMaxMin !== undefined ? c.activeMaxMin : 720;
    function formatActiveRange(min, max) {
      var m = Math.floor(min), M = Math.floor(max);
      var mLabel, MLabel;
      if (m < 60) mLabel = m + " 分钟";
      else mLabel = (m / 60).toFixed(m % 60 === 0 ? 0 : 1) + " 小时";
      if (M < 60) MLabel = M + " 分钟";
      else MLabel = (M / 60).toFixed(M % 60 === 0 ? 0 : 1) + " 小时";
      return mLabel + " ~ " + MLabel;
    }
    var activeLabel = formatActiveRange(actMinMin, actMaxMin);
    html += '<div class="group-head">主动消息</div>';
    html += '<div class="reply-settings">' +
      '<div class="reply-setting-row">' +
        '<div class="reply-setting-info">' +
          '<span class="func-title">最短间隔</span>' +
          '<span class="func-sub" id="active-min-label">' + (actMinMin < 60 ? actMinMin + " 分钟" : (actMinMin / 60).toFixed(actMinMin % 60 === 0 ? 0 : 1) + " 小时") + '</span>' +
        '</div>' +
        '<div class="slider-wrap">' +
          '<input type="range" class="fog-slider" id="active-min-slider" ' +
          'min="1" max="720" step="1" value="' + actMinMin + '">' +
        '</div>' +
      '</div>' +
      '<div class="reply-setting-row">' +
        '<div class="reply-setting-info">' +
          '<span class="func-title">最长间隔</span>' +
          '<span class="func-sub" id="active-max-label">' + (actMaxMin < 60 ? actMaxMin + " 分钟" : (actMaxMin / 60).toFixed(actMaxMin % 60 === 0 ? 0 : 1) + " 小时") + '</span>' +
        '</div>' +
        '<div class="slider-wrap">' +
          '<input type="range" class="fog-slider" id="active-max-slider" ' +
          'min="1" max="720" step="1" value="' + actMaxMin + '">' +
        '</div>' +
      '</div>' +
      '<div class="reply-hint">对方会在此区间内随机时间主动发消息 · 发送完毕后重新计时 · 1 分钟 ~ 12 小时</div>' +
      '</div>';
  ...
  function bindProfile(data) {
    ...
    // 主动消息时间区间滑块
    var actMinSlider = pageEl.querySelector("#active-min-slider");
    var actMaxSlider = pageEl.querySelector("#active-max-slider");
    var actMinLabel = pageEl.querySelector("#active-min-label");
    var actMaxLabel = pageEl.querySelector("#active-max-label");
    function fmtActiveMin(m) {
      if (m < 60) return m + " 分钟";
      return (m / 60).toFixed(m % 60 === 0 ? 0 : 1) + " 小时";
    }
    if (actMinSlider && actMinLabel) {
      actMinSlider.addEventListener("input", function () {
        var val = parseInt(this.value, 10);
        // 实时校验：最短不超过最长
        var maxVal = actMaxSlider ? parseInt(actMaxSlider.value, 10) : val;
        if (val > maxVal) {
          val = maxVal;
          this.value = val;
        }
        actMinLabel.textContent = fmtActiveMin(val);
      });
      actMinSlider.addEventListener("change", function () {
        var val = parseInt(this.value, 10);
        // 确保最小值不超过最大值
        var maxVal = actMaxSlider ? parseInt(actMaxSlider.value, 10) : val;
        if (val > maxVal) {
        ...
        save();
        // 通知 chat 模块重新调度
        if (window.MineChat && MineChat.rescheduleActiveMsg) {
          MineChat.rescheduleActiveMsg(c.id);
        }
      });
    }
    if (actMaxSlider && actMaxLabel) {
      actMaxSlider.addEventListener("input", function () {
        var val = parseInt(this.value, 10);
        // 实时校验：最长不小于最短
        var minVal = actMinSlider ? parseInt(actMinSlider.value, 10) : val;
        if (val < minVal) {
          val = minVal;
          this.value = val;
        }
        actMaxLabel.textContent = fmtActiveMin(val);
      });
      actMaxSlider.addEventListener("change", function () {
        var val = parseInt(this.value, 10);
        // 确保最大值不小于最小值
        var minVal = actMinSlider ? parseInt(actMinSlider.value, 10) : val;
        if (val < minVal) {
