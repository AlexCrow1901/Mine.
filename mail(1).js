...
window.MineMail = (function () {
  ...
  */
  ...
  /* ==================== AI 回复通道（预留接口） ====================
     后续接入 AI API 时，只需设置：
       MineMail.configureAI({ enabled: true, apiKey: "...", apiUrl: "..." });
     未启用或未配置时，自动回退为字卡组合回复。
  */
  var AIConfig = {
    enabled: false,
    apiKey: null,
  ...
  */
  ...
  function generateReply(pending) {
    // 50% 概率选择 AI，但 AI 未启用时自动回退字卡
    var useAI = Math.random() < 0.5;

    if (useAI && AIConfig.enabled && AIConfig.apiKey) {
      // AI 通道（预留：后续接入 API）
      return generateAIReply(pending);
    }
    // 字卡组合回复
    return generateCardReply(pending.recipient);
  }

  /* 字卡组合回复：抽取该联系人字卡总量的 5%-20%，随机标点衔接 */
  function generateCardReply(recipient) {
    ...
    // 5%-20% 区间随机
    var minCount = Math.max(1, Math.ceil(total * 0.05));
    var maxCount = Math.max(minCount, Math.ceil(total * 0.20));
  ...
  /* AI 回复（预留接口，当前返回占位文本） */
  function generateAIReply(pending) {
    // 后续接入 AI API：
    // fetch(AIConfig.apiUrl, { ... headers: { Authorization: AIConfig.apiKey } ... })
    //   .then(res => res.json())
    //   .then(data => { ... })
  ...
  return {
    ...
    onSend: null,
    // 配置 AI 回复通道
    configureAI: function (config) {
      if (!config) return;
      AIConfig.enabled = config.enabled !== undefined ? config.enabled : AIConfig.enabled;
      AIConfig.apiKey = config.apiKey || AIConfig.apiKey;
      AIConfig.apiUrl = config.apiUrl || AIConfig.apiUrl;
      AIConfig.model = config.model || AIConfig.model;
    },
    // 获取 AI 配置状态
    getAIConfig: function () {
      return { enabled: AIConfig.enabled, hasKey: !!AIConfig.apiKey, apiUrl: AIConfig.apiUrl };
    },
    // 接收邮件（供其他模块调用，模拟收到来信）
