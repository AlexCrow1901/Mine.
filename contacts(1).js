...
window.MineContacts = (function () {
    ...
    try {
      ...
      if (raw) {
        ...
        state.contacts.forEach(function (c) {
          ...
          // 按拼音首字母排序字卡
          if (U && c.cards.length > 1) c.cards = U.sortByPinyin(c.cards);
          // 去重 + 排序：自动回复字卡
        ...
        state.groups.forEach(function (g) {
          ...
          // 按拼音首字母排序群字卡
          if (U && g.cards.length > 1) g.cards = U.sortByPinyin(g.cards);
          // 去重 + 排序：群自动回复字卡
  ...
  function viewProfile(data) {
    ...
    html += '<div class="reply-settings">' +
      '<div class="reply-setting-row">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="reply-hint">3% 概率触发自动回复 · 回复时间在 0 ~ 设定值内随机 · 拖动调节 0 秒 ~ 10 分钟</div>' +
      '</div>';
    ...
    html += '<div class="card-hint">3% 不回复时自动发送 · 标记「自动回复」· 空格分隔批量添加 · 自动去重 · A→Z 排序</div>';

    html += '<div class="list-sep"></div>';

    // 字卡区
    html += '<div class="group-head">字卡 <span class="count">' + (c.cards || []).length + '</span>';
    if ((c.cards || []).length > 0) {
      html += '<button class="batch-toggle-btn" data-act="toggle-batch">' + I.svg("trash", 14) + ' 批量删除</button>';
    }
    html += '</div>';
    html += '<div class="cards-section" id="cards-section">';
    html += renderCardsList(c, false);
    html += '</div>';
    ...
    html += '<div class="card-add-row">' +
      ...
      'placeholder="输入字卡，空格分隔可批量添加… 自动去重">' +
      '<button class="btn card-add-btn" data-act="add-card">' + I.svg("plus", 18) + '</button>' +
      '</div>';

    html += '<div class="card-hint">空格分隔可一次添加多条 · 自动去重 · 按拼音首字母 A→Z 排序 · 无数量限制</div>';

    html += '</div>';
    return html;
  }

  function renderCardsList(c, batchMode) {
    var cards = c.cards || [];
    if (cards.length === 0) {
    ...
    cards.forEach(function (text, i) {
      if (batchMode) {
        html += '<div class="card-item card-selectable" data-card-idx="' + i + '">' +
  ...
  function renderAutoCardsList(c, batchMode) {
    ...
    if (cards.length === 0) {
      return '<div class="cards-empty">暂无自动回复字卡，3% 不回复时将保持沉默</div>';
    }
    var html = "";
    cards.forEach(function (text, i) {
      if (batchMode) {
  ...
  function bindProfile(data) {
    ...
    var addBtn = pageEl.querySelector('[data-act="add-card"]');
    var batchSelected = {};  // 批量选中状态 { idx: true }

    function refreshCards(batchMode) {
      pageEl.querySelector("#cards-section").innerHTML = renderCardsList(c, batchMode);
    ...
    function doAddCard() {
      ...
      // 按拼音首字母 A→Z 排序
      if (window.MineUtils) {
        c.cards = window.MineUtils.sortByPinyin(c.cards);
      }
      addInput.value = "";
      save();
      // 退出批量模式（如果开着的话）
    ...
    addBtn.addEventListener("click", doAddCard);
  ...
  function viewGroupDetail(data) {
    ...
    // 群字卡区
    html += '<div class="group-head">群字卡 <span class="count" id="group-card-count">' + (g.cards || []).length + '</span>';
    if ((g.cards || []).length > 0) {
      html += '<button class="batch-toggle-btn" data-act="toggle-gcard-batch">' + I.svg("trash", 14) + ' 批量删除</button>';
    }
    html += '</div>';
    html += '<div class="cards-section" id="group-cards-section">';
    html += renderGroupCardsList(g, false);
    html += '</div>';
    ...
    html += '<div class="card-add-row">' +
      ...
      'placeholder="输入群字卡，空格分隔可批量添加… 自动去重">' +
      '<button class="btn card-add-btn" data-act="add-group-card">' + I.svg("plus", 18) + '</button>' +
      '</div>';

    html += '<div class="card-hint">空格分隔可一次添加多条 · 自动去重 · 按拼音首字母 A→Z 排序 · 群字卡可被所有成员在群聊中使用</div>';
  ...
  /* -------- 群字卡渲染与绑定 -------- */
  function renderGroupCardsList(g, batchMode) {
    var cards = g.cards || [];
    if (cards.length === 0) {
    ...
    cards.forEach(function (text, i) {
      if (batchMode) {
        html += '<div class="card-item card-selectable" data-gcard-idx="' + i + '">' +
          '<span class="card-check">' + I.svg("check", 14) + '</span>' +
          '<span class="card-text">' + escapeHtml(text) + '</span>' +
          '</div>';
      } else {
  ...
  function renderGroupAutoCardsList(g, batchMode) {
    ...
    if (cards.length === 0) {
      return '<div class="cards-empty">暂无群自动回复字卡，3% 不回复时将保持沉默</div>';
    }
    var html = "";
    cards.forEach(function (text, i) {
      if (batchMode) {
  ...
  function bindGroupCards(data) {
    ...
    var addBtn = pageEl.querySelector('[data-act="add-group-card"]');
    var gcardBatchSelected = {};

    function refreshGroupCards(batchMode) {
      pageEl.querySelector("#group-cards-section").innerHTML = renderGroupCardsList(g, batchMode);
    ...
    function doAddCard() {
      ...
      // 按拼音首字母 A→Z 排序
      if (window.MineUtils) {
        g.cards = window.MineUtils.sortByPinyin(g.cards);
      }
      addInput.value = "";
      save();
      // 退出批量模式（如果开着的话）
    ...
    pageEl.querySelectorAll("[data-del-gcard]").forEach(function (btn) {
      btn.addEventListener("click", function (e) {
        ...
        if (g.cards && g.cards.length > idx) {
          g.cards.splice(idx, 1);
          save();
          pageEl.querySelector("#group-cards-section").innerHTML = renderGroupCardsList(g);
          var countEl = pageEl.querySelector("#group-card-count");
          if (countEl) countEl.textContent = (g.cards || []).length;
          bindGroupCardDels(data);
        }
