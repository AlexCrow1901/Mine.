...
window.MineChat = (function () {
  ...
  /* ========================================================================
     后台任务队列（核心保活机制）
     原理：所有定时回复任务同时注册到队列 + setTimeout 双通道执行，
     即使后台 setTimeout 被浏览器冻结，页面恢复可见时也能补执行。
  ------------------------------------------------------------------------ */
  var pendingBgTasks = [];  // [{ executeTime, fn, executed }]

  /* 后台安全的定时执行：同时走 setTimeout（前台立即执行）+ 任务队列（后台补偿执行） */
  function bgSchedule(fn, delay) {
    var task = { executeTime: Date.now() + delay, fn: fn, executed: false };
    pendingBgTasks.push(task);
    // 同时用 setTimeout（前台时能即时执行）
    setTimeout(function () {
      if (task.executed) return;  // 已被队列补偿执行过，跳过
      task.executed = true;
      try { fn(); } catch (e) {}
    }, delay);
  }

  /* 补偿执行：检查所有已到期但未执行的任务，立即执行 */
  function processPendingBgTasks() {
    var now = Date.now();
    for (var i = 0; i < pendingBgTasks.length; i++) {
      var t = pendingBgTasks[i];
      if (!t.executed && t.executeTime <= now) {
        t.executed = true;
        try { t.fn(); } catch (e) {}
      }
    }
    // 清理已执行的任务
    pendingBgTasks = pendingBgTasks.filter(function (t) { return !t.executed; });
  }

  /* ========================================================================
     主动消息调度系统
     原理：为每个联系人群成员调度一条主动消息，在设定的时间区间内随机触发。
     触发后发送 1~4 条字卡（按概率分布），发送完毕后自动重新调度下一条。
     使用 localStorage 持久化，刷新页面后恢复调度。
  ------------------------------------------------------------------------ */
  var ACTIVE_MSG_KEY = "mine.chat.activeMsg.v1";
  var pendingActiveMsgs = [];  // [{ contactId, groupId, convKey, nextSendTime, scheduled }]

  function loadActiveMsgs() {
    try {
      var raw = localStorage.getItem(ACTIVE_MSG_KEY);
      if (raw) pendingActiveMsgs = JSON.parse(raw) || [];
    } catch (e) {}
    if (!pendingActiveMsgs) pendingActiveMsgs = [];
    // 兼容旧数据：给没有 scheduleId 的项补一个
    pendingActiveMsgs.forEach(function (p) {
      if (!p.scheduleId) p.scheduleId = genScheduleId();
    });
  }

  function saveActiveMsgs() {
    try {
      localStorage.setItem(ACTIVE_MSG_KEY, JSON.stringify(pendingActiveMsgs));
    } catch (e) {}
  }

  /* 主动消息的字卡抽取：按用户设定的概率分布
     - 60% 1条, 30% 2条, 7% 3条, 3% 3条以上（取 4 条）
     - 每条字卡：90% 文字字卡, 10% 图片字卡
     - 文字字卡有 15% 概率附加 1~3 条 emoji（73%/20%/7%）
  */
  function pickActiveMsgCards(pool) {
    var cards = [];
    var imageCards = filterImageCards(pool);
    var textCards = filterTextCards(pool);
    var emojiCards = filterEmojiCards(pool);
    ...
    for (var i = 0; i < count; i++) {
      var isImage = imageCards.length > 0 && Math.random() < 0.10;
      if (isImage) {
        cards.push(imageCards[Math.floor(Math.random() * imageCards.length)]);
      } else if (textCards.length > 0) {
        var card = textCards[Math.floor(Math.random() * textCards.length)];
        cards.push(card);
        // 文字字卡有 15% 概率附加 emoji
        if (emojiCards.length > 0 && Math.random() < 0.15) {
          var er = Math.random();
          var emojiCount = er < 0.73 ? 1 : (er < 0.93 ? 2 : 3);
          for (var j = 0; j < emojiCount; j++) {
            cards.push(emojiCards[Math.floor(Math.random() * emojiCards.length)]);
          }
        }
  ...
  /* 执行主动消息发送 */
  function doActiveMessage(item) {
    if (!item || !item.convKey) return;
    var msgs = conversations[item.convKey];
    if (!msgs) msgs = [];
    ...
    if (item.groupId) {
      // 群聊：从群成员中随机选一个人发
      var group = C.findGroup(item.groupId);
      var member = C.findContact(item.contactId);
      if (member) {
        senderName = member.name || "未知";
        senderAvatar = member.avatar || null;
        fromId = member.id || "unknown";
        if (member.cards) cardPool = cardPool.concat(member.cards);
      }
      if (group && group.cards) cardPool = cardPool.concat(group.cards);
    } else {
      // 一对一
      var contact = C.findContact(item.contactId);
      if (contact) {
        senderName = contact.name || "未知";
        senderAvatar = contact.avatar || null;
        fromId = contact.id || "unknown";
        if (contact.cards) cardPool = cardPool.concat(contact.cards);
      }
    }

    if (cardPool.length === 0) return;

    var cards = pickActiveMsgCards(cardPool);
    ...
    for (var k = 0; k < cards.length; k++) {
      msgs.push({
        ...
        from: fromId,
        text: cards[k],
        time: Date.now() + k * 2000,  // 每条间隔 2 秒
        senderName: senderName,
        senderAvatar: senderAvatar
      });
    }

    conversations[item.convKey] = msgs;
    var saved = save();

    // 未读计数
    if (saved && !isViewingConv(item.convKey)) {
      incrementUnread(item.convKey);
      if (window.MineNotify) MineNotify.refreshBadges();
    }

    // 安全追加 DOM
    var newMsgs = msgs.slice(beforeLen);
    safeAppendMessages(item.convKey, newMsgs);

    /* 电话概率触发：后台主动消息发送字卡后同样触发
       仅对一对一会话（非群聊）触发 */
    if (!item.groupId && fromId !== "unknown" && window.MinePhone) {
      MinePhone.maybeTriggerIncomingCall(fromId, false);
      if (Math.random() < 0.002) {
        var _allContacts = (C && C.getState) ? (C.getState().contacts || []) : [];
        if (_allContacts.length > 1) {
          var _others = _allContacts.filter(function (c) { return c.id !== fromId; });
          if (_others.length > 0) {
            var _randomOther = _others[Math.floor(Math.random() * _others.length)];
            setTimeout(function () {
              if (window.MinePhone) MinePhone.maybeTriggerIncomingCall(_randomOther.id, true);
  ...
  /* 为指定联系人/群成员调度下一条主动消息 */
  function scheduleNextActiveMsg(contactId, groupId, convKey) {
    if (!C || !C.findContact) return;
    var contact = C.findContact(contactId);
    if (!contact) return;

    // 获取时间区间（分钟）
    var minMin = contact.activeMinMin !== undefined ? contact.activeMinMin : 1;
    var maxMin = contact.activeMaxMin !== undefined ? contact.activeMaxMin : 720;
    if (minMin < 1) minMin = 1;
    if (maxMin < minMin) maxMin = minMin;

    // 区间内随机延迟
    var delayMin = minMin + Math.random() * (maxMin - minMin);
    var delayMs = delayMin * 60 * 1000;
    var nextSendTime = Date.now() + delayMs;

    // 先移除旧的调度
    pendingActiveMsgs = pendingActiveMsgs.filter(function (p) {
      return !(p.contactId === contactId && p.groupId === (groupId || null) && p.convKey === convKey);
    });

    var schedId = genScheduleId();
    var item = {
      contactId: contactId,
    ...
    pendingActiveMsgs.push(item);
    saveActiveMsgs();

    // 用 bgSchedule 调度（支持后台补偿执行）
    bgSchedule(function () {
      // 通过 scheduleId 精确匹配，防止旧定时器误触发新调度项
      var p = pendingActiveMsgs.find(function (x) {
        return x.scheduleId === schedId;
      });
      if (!p || p.executed) return;
      p.executed = true;

      doActiveMessage(p);

      // 发送完毕后重新调度下一条
      scheduleNextActiveMsg(contactId, groupId, convKey);
    }, delayMs);
  }

  /* 启动所有主动消息调度（页面加载时调用） */
  function startActiveMsgScheduler() {
    loadActiveMsgs();
    ...
    pendingActiveMsgs.forEach(function (p) {
      ...
      var remaining = p.nextSendTime - now;
      var schedId = p.scheduleId;
      if (remaining <= 0) {
        // 已到期，立即执行
        p.executed = true;
        doActiveMessage(p);
        scheduleNextActiveMsg(p.contactId, p.groupId, p.convKey);
      } else {
        // 还没到，重新调度
        bgSchedule(function () {
          // 通过 scheduleId 精确匹配，防止旧定时器误触发
          var item = pendingActiveMsgs.find(function (x) {
            return x.scheduleId === schedId;
          });
          if (!item || item.executed) return;
          item.executed = true;
          doActiveMessage(item);
          scheduleNextActiveMsg(item.contactId, item.groupId, item.convKey);
        }, remaining);
      }
    });
  }
  ...
  /* 新增会话时检查是否需要启动主动消息调度 */
  function ensureActiveMsgForConv(convKey, type, id) {
    if (!C || !C.loadData) return;
    C.loadData();

    if (type === "contact") {
      // 一对一：检查是否已有调度，没有则启动
      var existing = pendingActiveMsgs.find(function (p) {
        return p.convKey === convKey && !p.groupId;
      });
      if (!existing) {
        scheduleNextActiveMsg(id, null, convKey);
      }
    } else if (type === "group") {
      // 群聊：为每个成员启动调度
      var group = C.findGroup(id);
      if (group && group.members) {
        group.members.forEach(function (m) {
          var memberId = typeof m === "string" ? m : m.id;
          var member = C.findContact(memberId);
          if (!member) return;
          var gExisting = pendingActiveMsgs.find(function (p) {
            return p.convKey === convKey && p.groupId === id && p.contactId === memberId;
          });
          if (!gExisting) {
            scheduleNextActiveMsg(memberId, id, convKey);
          }
        });
      }
    }
  ...
  function openContact(contactId) {
    ...
    // 确保主动消息调度已启动
    ensureActiveMsgForConv(ctx.convKey, ctx.type, ctx.id);
    renderChatPage();
      ...
      '<div class="with-me-bar" id="with-me-bar">' +
        ...
        '<div class="with-me-panel" id="with-me-panel">' +
          '<div class="with-me-content" id="with-me-content">' +
            ...
            '</div>' +
            '<div class="wm-action" role="button" tabindex="0" data-wm="phone">' +
              '<span class="wm-action-icon">' + I.svg("phone", 18) + '</span>' +
              '<span class="wm-action-name">电话</span>' +
            '</div>' +
          '</div>' +
          '</div>' +
        '</div>' +
  ...
  function bindChatEvents() {
    ...
    // 电话功能：直接拨打给当前联系人
    var phoneBtn = pageEl.querySelector('[data-wm="phone"]');
    if (phoneBtn) {
      phoneBtn.addEventListener("click", function () {
        if (ctx.type === "contact" && ctx.id && window.MinePhone) {
          MinePhone.startOutgoingCall(ctx.id);
          if (window.MineApp && MineApp.switchPage) MineApp.switchPage("phone");
        }
      });
  ...
  */
      ...
      responders.forEach(function (member) {
        ...
        (function (m, d) {
          ...
          if (d > 15000) {
            ...
            bgSchedule(doReply, d);
          } else {
            // 延迟很短：立即显示"正在输入"
            setTimeout(function () {
              if (isViewingConv(curConvKey)) {
                showTyping(m);
                bgSchedule(doReply, Math.max(300, d));
              } else {
    ...
    if (actualDelay > 15000) {
      ...
      setTimeout(function () {
        if (isViewingConv(convKey)) {
          ...
          // 不在查看时直接调度回复
          bgSchedule(function () { doAutoReply(contact, card, convKey); }, Math.max(300, actualDelay));
        }
      }, 0);
    }
  }
  ...
  function doContactReply(contact, convKey) {
    ...
    /* 电话概率触发：对方每发来一张字卡
       - 1% 概率：当前联系人主动来电
       - 0.2% 概率：列表其他联系人主动来电 */
    if (card && window.MinePhone && fromId !== "unknown") {
      MinePhone.maybeTriggerIncomingCall(fromId, false);
      if (Math.random() < 0.002) {
        var allContacts = (C && C.getState) ? (C.getState().contacts || []) : [];
        if (allContacts.length > 1) {
          var others = allContacts.filter(function (c) { return c.id !== fromId; });
          if (others.length > 0) {
            var randomOther = others[Math.floor(Math.random() * others.length)];
            setTimeout(function () {
              if (window.MinePhone) MinePhone.maybeTriggerIncomingCall(randomOther.id, true);
            }, 1000 + Math.random() * 2000);
          }
        }
      }
  ...
  /* 心跳定时器：定期补偿执行后台任务 + 检查待渲染队列 */
  var heartbeatTimer = setInterval(function () {
    // ★ 核心：补偿执行所有到期的后台任务（即使 setTimeout 被冻结也能执行）
    processPendingBgTasks();
    // 处理待渲染队列
    if (ctx.convKey) {
      flushPendingRender();
    }
  ...
  document.addEventListener("visibilitychange", function () {
    if (!document.hidden) {
      // 页面重新可见：启动保活 + 补偿执行 + 消费待渲染队列 + 刷新当前会话
      startKeepAlive();
      // ★ 核心：立即补偿执行所有后台冻结期间应该执行但没执行的回复
      processPendingBgTasks();
      // 消费待渲染队列
      if (ctx.convKey) {
        // 在聊天界面内 → 重新渲染消息，立即看到对方回复
        setTimeout(function () {
          flushPendingRender();
          try {
            if (msgContainer) renderMessages();
        ...
        // 在会话列表页 → 刷新列表，显示最新消息预览 + 未读红点（类似微信）
        var chatPage = document.getElementById("page-chat");
        if (chatPage && chatPage.classList.contains("is-active")) {
          try { openConvList(); } catch (e) {}
        }
      }
      // 刷新会话列表的未读标记
      try {
  ...
  return {
    ...
    /* 重新调度某个联系人的主动消息（修改时间区间后调用） */
    rescheduleActiveMsg: function (contactId) {
      if (!contactId) return;
      // 找到所有该联系人的主动消息调度，删除后重新调度
      var toReschedule = pendingActiveMsgs.filter(function (p) {
      ...
      saveActiveMsgs();
      toReschedule.forEach(function (p) {
        scheduleNextActiveMsg(p.contactId, p.groupId, p.convKey);
      });
    }
  };
