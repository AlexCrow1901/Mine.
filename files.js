/* ========================================================================
   ...
   ------------------------------------------------------------------------
   · 存储用量统计
   · 数据导出（聊天 / 信件 / 树洞 / 朋友圈 / 全部）
   · 数据清除（分类清除 / 全部清除）
   · 从个人中心迁移至此，通过主页"文件"入口进入
   ======================================================================== */
...
window.MineFiles = (function () {
  ...
  function renderPage() {
    ...
    // 分类清理按钮
    html += '<div class="group-head">清除数据</div>';
  ...
  function bindEvents(pageEl) {
    ...
    if (clearAllBtn) {
      var clearAllHandler = function () {
        ...
        /* 使用 MineStore 统一清除（含 IndexedDB 独有的键） */
        if (window.MineStore && MineStore.clearAllMine) {
          MineStore.clearAllMine(keepKeys);
          /* 确保也清除电台 IndexedDB 数据 */
          if (window.MineRadio && MineRadio.clearAll) {
            try { MineRadio.clearAll(); } catch (e) {}
          }
        } else {
          var toRemove = [];
  ...
  function updateStorageInfo(pageEl) {
    ...
    var totalKB, maxKB, percent, maxMod;

    /* 优先使用 MineStore 统一统计（含 IndexedDB 数据） */
    if (window.MineStore && MineStore.getUsage) {
      var usage = MineStore.getUsage();
      totalKB = usage.totalKB;
      maxKB = usage.maxKB;
      percent = Math.min(100, Math.round(usage.totalSize / (maxKB * 1024) * 100));

      maxMod = "";
      var maxModSize = 0;
      Object.keys(usage.moduleSizes).forEach(function (mod) {
        if (usage.moduleSizes[mod] > maxModSize) {
          maxModSize = usage.moduleSizes[mod];
          maxMod = mod;
        }
      });
    } else {
      ...
      maxKB = 5120;
      percent = Math.min(100, Math.round(totalSize / (maxKB * 1024) * 100));

      maxMod = "";
      var maxModSize2 = 0;
      Object.keys(moduleSizes).forEach(function (mod) {
        if (moduleSizes[mod] > maxModSize2) {
          maxModSize2 = moduleSizes[mod];
          maxMod = mod;
        }
      });
    }

    infoEl.textContent = "已用 " + totalKB + " KB / " + maxKB + " KB (" + percent + "%)" +
      (maxMod ? " · " + maxMod + " 占用最多" : "");
